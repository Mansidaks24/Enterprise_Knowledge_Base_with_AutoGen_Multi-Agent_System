...
class EnterpriseKnowledgeBase:
    ...
    def __init__(
        ...
        self.session_id = "enterprise_default_session"

        # Restore memory if exists
        stored_history = self.memory.load_session(
            self.session_id
        )

        if stored_history:

            self.conversation_history = stored_history

            print("✓ Previous session memory restored")
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        final_answer = self.synthesizer.synthesize_answer(
            query,
            analysis_result.analysis,
            self.fact_checker.generate_validation_report_text(
                validation_report
            ),
        ...
        # Format final output
        final_output = self.synthesizer.format_for_presentation(
            final_answer
        )
        ...
        response = {
            ...
            "processing_steps": {
                ...
                "query_routing": {
                    ...
                    "routing_parameters":
                        query_analysis.routing_parameters
                },

                "data_retrieval": {

                    "source_type":
                        retrieval_result.source_type,

                    "confidence":
                        retrieval_result.confidence,

                    "items_found":
                        len(retrieval_result.metadata)
                },

                "analysis": {

                    "findings_count":
                        len(analysis_result.key_findings),
        ...
        try:
            ...
            print(
                f"⚠️ Memory persistence failed: {e}"
            )
    ...
    def export_conversation_history(
        ...
        print(
            f"✓ Conversation history exported "
            f"to {filepath}"
        )
    ...
    def export_all_logs(
        ...
        log_files['retrieval'] = (
            f"{output_dir}/"
            f"retrieval_log_{self.session_id}.json"
        )

        self.retriever.export_retrieval_log(
            log_files['retrieval']
        )

        log_files['analysis'] = (
            f"{output_dir}/"
            f"analysis_log_{self.session_id}.json"
        )

        self.analyst.export_analysis_log(
            log_files['analysis']
        )

        log_files['validation'] = (
            f"{output_dir}/"
            f"validation_log_{self.session_id}.json"
        )

        self.fact_checker.export_validation_log(
            log_files['validation']
        )

        log_files['synthesis'] = (
            f"{output_dir}/"
            f"synthesis_log_{self.session_id}.json"
        )

        self.synthesizer.export_synthesis_log(
            log_files['synthesis']
        )

        log_files['routing'] = (
            f"{output_dir}/"
            f"routing_log_{self.session_id}.json"
        )

        self.query_router.export_routing_log(
            log_files['routing']
        )

        log_files['conversation'] = (
            f"{output_dir}/"
